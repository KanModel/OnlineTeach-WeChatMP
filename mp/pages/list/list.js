...
Page({
    data: {
        ...
        return {
            title: title, path: path, success: function (res) {/* 转发成功*/
    ...
    /*加载分页*/ loadMore: function (e) {
        ...
        if (options.search && options.search != '') {
            wx.setNavigationBarTitle({
                title: "搜索关键字：" + options.search, success: function (res) {/* success*/
                }
            });
        ...
        /* if (!data.categories) data.categories = 0; if (!data.search) data.search = '';*/
        if (data.page === 1) {
        ...
        wx.request({
            ...
            data: {no: data.page, size: pageCount, title: data.search},
        ...
        getCategoryRequest.then(response => {
            ...
            wx.setNavigationBarTitle({
                title: response.data.name, success: function (res) {/* success*/
                }
            });
